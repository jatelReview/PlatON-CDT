...
namespace platon {
     ...
     *
     */
#define platon_assert(A, ...) ::platon::assert_aux(A, #A, __LINE__, __FILE__, __func__, ##__VA_ARGS__)
    /**
...
#define platon_assert_ne(A, B, ...) platon_assert(((A)!=(B)), ##__VA_ARGS__)
    ...
    inline void assert_aux(bool cond, const char *condStr, unsigned line, const char *file, const char *func, Args&&... args) {
        if (!cond) {
            println("Assertion failed:", condStr, "func:", func, "line:", line, "file:", file, args...);
            ::platon_panic();
        }
    }
}
