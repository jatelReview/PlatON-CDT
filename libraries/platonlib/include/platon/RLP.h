...
class RLP {
 public:
  ...
  bool toBool(int _flags = Strict) const {
    uint8_t _i = toInt<uint8_t>(_flags);
    return static_cast<bool>(_i);
  }
  ...
  float toFloat(int _flags = Strict) const {
    uint32_t _i = toInt<uint32_t>(_flags);
    return *reinterpret_cast<float*>(&_i);
  }

  double toDouble(int _flags = Strict) const {
    uint64_t _i = toInt<uint64_t>();
    return *reinterpret_cast<double*>(&_i);
  }
...
struct Converter<bool> {
  static bool convert(RLP const& _r, int _flags) { return _r.toBool(_flags); }
};
template <>
...
struct Converter<int8_t> {
  static int8_t convert(RLP const& _r, int _flags) {
    return _r.toSignedInt<int8_t>(_flags);
  }
};
template <>
struct Converter<int16_t> {
  static int16_t convert(RLP const& _r, int _flags) {
    return _r.toSignedInt<int16_t>(_flags);
  }
};
template <>
...
struct Converter<int64_t> {
  static int64_t convert(RLP const& _r, int _flags) {
    return _r.toSignedInt<int64_t>(_flags);
  }
};
template <>
struct Converter<float> {
  static float convert(RLP const& _r, int _flags) { return _r.toFloat(_flags); }
};
template <>
struct Converter<double> {
  static double convert(RLP const& _r, int _flags) {
    return _r.toDouble(_flags);
  }
};
template <unsigned N>
...
class RLPStream {
 public:
  ...
  RLPStream& append(bool _b) {
    unsigned _s = _b ? 1 : 0;
    return append(bigint(_s));
  }
  // RLPStream& append(unsigned _s) { return append(bigint(_s)); }
  RLPStream& append(uint8_t _s) { return append(bigint(_s)); }
  RLPStream& append(uint16_t _s) { return append(bigint(_s)); }
  ...
  // RLPStream& append(long _s) { return append(int128_t(_s)); }
  ...
  RLPStream& append(float _f) {
    uint32_t _i = *reinterpret_cast<uint32_t*>(&_f);
    return append(_i);
  }
  RLPStream& append(double _d) {
  ...
  template <class _T>
  RLPStream& operator<<(std::set<_T> const& _s);

  template <class _T>
  RLPStream& operator<<(std::unordered_set<_T> const& _s);

  template <class T, class U>
  RLPStream& operator<<(std::pair<T, U> const& _s);

  template <class T, class U>
  RLPStream& operator<<(std::map<T, U> const& _s);
